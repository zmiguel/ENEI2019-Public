...
function renderCurrentSentence(state) {
  ...
  return p('.current-sentence', [!state.previous ? WELCOME_SENTENCE : null, state.previous ? '"' + state.previous + (state.options.length === 1 ? '.' : '...') + '"' : null, state.previous ? span('.undo', '\u21A9\xA0Undo') : null, state.previous ? span('.reset', 'Or\xA0reset') : null].filter(function (x) {
    return x !== null;
  }));
}
...
function renderStaticDecision(option) {
  ...
  return h4('.decision', ['\xBB You want the static operator ', a({ attrs: { href: OBSERVABLE_PATH + '#static-method-' + label } }, label), '.']);
}

function renderInstanceDecision(option) {
  return h4('.decision', ['\xBB You want the instance operator ', a({ attrs: { href: OBSERVABLE_PATH + '#instance-method-' + option.label } }, option.label), '.']);
}
